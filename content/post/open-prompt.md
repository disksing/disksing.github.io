...
1. **空 PR 启动**：我们不再从本地秘密开发开始，而是直接先开一个公开的空 PR。

2. **公开的 Agent 会话**：在 PR 页面通过 Comment 指挥 Coding Agent 写代码。每个 PR 背后都绑定着一个长期的 Agent Session，Agent 读取指令、修改代码并自动推送到当前分支。Agent 的输出、试错和代码变更都在评论区全公开。

3. **过程即代码**：写得差不多了就把 Reviewer 拉进来。Reviewer 看到的不只是 Diff，更是整个生成代码的过程。
